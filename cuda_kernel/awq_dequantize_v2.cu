...
__global__ void awq_dequantize_v2_kernel(
...
torch::Tensor awq_dequantize_v2_cuda(
    ...
    const int num_blocks = (total_packed_elements + threads_per_block - 1) / threads_per_block;

    awq_dequantize_v2_kernel<<<num_blocks, threads_per_block>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("awq_dequantize_v2", &awq_dequantize_v2_cuda, "Optimized AWQ dequantization (CUDA)");
}
